...
public typealias Completion = (_ result: Result<DDMoya.Response, MoyaError>) -> Void
...
public protocol MoyaProviderType: AnyObject {
    ...
    func request(_ target: Target, callbackQueue: DispatchQueue?, progress: DDMoya.ProgressBlock?, completion: @escaping DDMoya.Completion) -> Cancellable
}
...
open class MoyaProvider<Target: TargetType>: MoyaProviderType {
    ...
    public typealias StubClosure = (Target) -> DDMoya.StubBehavior
    ...
    open internal(set) var inflightRequests: [Endpoint: [DDMoya.Completion]] = [:]
    ...
    @discardableResult
    open func stubRequest(_ target: Target, request: URLRequest, callbackQueue: DispatchQueue?, completion: @escaping DDMoya.Completion, endpoint: Endpoint, stubBehavior: DDMoya.StubBehavior) -> CancellableToken {
        let callbackQueue = callbackQueue ?? self.callbackQueue
...
public extension MoyaProvider {
    ...
    /// Do not stub.
    final class func neverStub(_: Target) -> DDMoya.StubBehavior {
        return .never
    }

    /// Return a response immediately.
    final class func immediatelyStub(_: Target) -> DDMoya.StubBehavior {
        return .immediate
    }

    /// Return a response after a delay.
    final class func delayedStub(_ seconds: TimeInterval) -> (Target) -> DDMoya.StubBehavior {
...
public func convertResponseToResult(_ response: HTTPURLResponse?, request: URLRequest?, data: Data?, error: Swift.Error?) ->
    Result<DDMoya.Response, MoyaError> {
        switch (response, data, error) {
        case let (.some(response), data, .none):
            let response = DDMoya.Response(statusCode: response.statusCode, data: data ?? Data(), request: request, response: response)

...
public extension MoyaProvider {
    ...
    func requestNormal(_ target: Target, callbackQueue: DispatchQueue?, progress: DDMoya.ProgressBlock?, completion: @escaping DDMoya.Completion) -> Cancellable {
        let endpoint = self.endpoint(target)
        let stubBehavior = self.stubClosure(target)
        let cancellableToken = CancellableWrapper()

        // Allow plugins to modify response
        let pluginsWithCompletion: DDMoya.Completion = { result in
        ...
        let performNetworking = { (requestResult: Result<URLRequest, MoyaError>) in
            ...
            let networkCompletion: DDMoya.Completion = { result in
    ...
    private func performRequest(_ target: Target, request: URLRequest, callbackQueue: DispatchQueue?, progress: DDMoya.ProgressBlock?, completion: @escaping DDMoya.Completion, endpoint: Endpoint, stubBehavior: DDMoya.StubBehavior) -> Cancellable {
        switch stubBehavior {
        case .never:
    ...
    func cancelCompletion(_ completion: DDMoya.Completion, target: Target) {
    ...
    final func createStubFunction(_ token: CancellableToken, forTarget target: Target, withCompletion completion: @escaping DDMoya.Completion, endpoint: Endpoint, plugins: [PluginType], request: URLRequest) -> (() -> Void) { // swiftlint:disable:this function_parameter_count
        return {
            ...
            let validate = { (response: DDMoya.Response) -> Result<DDMoya.Response, MoyaError> in
            ...
            case .networkResponse(let statusCode, let data):
                let response = DDMoya.Response(statusCode: statusCode, data: data, request: request, response: nil)
                let result = validate(response)
                plugins.forEach { $0.didReceive(result, target: target) }
                completion(result)
            case .response(let customResponse, let data):
                let response = DDMoya.Response(statusCode: customResponse.statusCode, data: data, request: request, response: customResponse)
                let result = validate(response)
                plugins.forEach { $0.didReceive(result, target: target) }
                completion(result)
...
private extension MoyaProvider {
    func sendUploadMultipart(_ target: Target, request: URLRequest, callbackQueue: DispatchQueue?, multipartBody: [MultipartFormData], progress: DDMoya.ProgressBlock? = nil, completion: @escaping DDMoya.Completion) -> CancellableWrapper {
    ...
    func sendRequest(_ target: Target, request: URLRequest, callbackQueue: DispatchQueue?, progress: DDMoya.ProgressBlock?, completion: @escaping DDMoya.Completion) -> CancellableToken {
    ...
    func sendAlamofireRequest<T>(_ alamoRequest: T, target: Target, callbackQueue: DispatchQueue?, progress progressCompletion: DDMoya.ProgressBlock?, completion: @escaping DDMoya.Completion) -> CancellableToken where T: Requestable, T: Request {
